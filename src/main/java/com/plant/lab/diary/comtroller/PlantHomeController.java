...
//		mv.addObject("likeList", dService.likeList(sessionVO));
	...
	public ModelAndView boardInsert(
			DiaryVO vo, /* @RequestParam(name = "upfile", required = false) MultipartFile report, */
			HttpSession session,
			HttpServletRequest request, @RequestParam(name="writetext") String diary_content, ModelAndView mv) {
		...
		try {
				...
				int result2= -1;
				/*
				 * if(report != null && !report.equals("")) saveFile(report, request);
	...
	@ResponseBody
	public void Insertcomment(HttpServletRequest request, CommentVO cvo, HttpSession session,
